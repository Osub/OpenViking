...
class ReindexExecutor:
    ...
    def _infer_target_type(self, uri: str) -> str:
        ...
        path = normalized_uri[len("viking://") :]
        parts = [part for part in path.split("/") if part]
        if normalized_uri == "viking://user":
        ...
        if normalized_uri.startswith("viking://user/"):
            ...
            if remainder and "/" not in remainder:
                return "user_namespace"
        if normalized_uri.startswith("viking://agent/"):
            remainder = normalized_uri[len("viking://agent/") :]
            if remainder and "/" not in remainder:
                return "agent_namespace"
        if (
            uri.startswith("viking://resources/")
            or uri.startswith("viking://user/")
            or uri.startswith("viking://agent/")
        ):
            return "resource"
        raise OpenVikingError(
            f"Unsupported reindex URI: {uri}",
    ...
    async def _reindex_user_namespace(
        ...
        *,
        ...
        for entry in entries:
            ...
            if not entry_uri:
                continue
            if "/memories" in entry_uri:
                if entry.get("isDir") and entry_uri.endswith("/memories"):
        ...
        for entry in entries:
            ...
            if "/memories" in entry_uri:
                if entry.get("isDir") and entry_uri.endswith("/memories"):
                    memory_roots.append(entry_uri)
                continue
            if "/skills" in entry_uri:
                if entry.get("isDir") and entry_uri.count("/skills/") == 1:
    ...
    async def _reindex_global_namespace(
        ...
        *,
        ...
        if mode == "semantic_and_vectors" and "viking://resources" in resource_directories:
    ...
    async def _upsert_context(
        ...
        *,
        ...
        context = Context(
            ...
            user=ctx.user,
            account_id=ctx.account_id,
            owner_space=self._owner_space_for_uri(uri, ctx),
            meta=meta or {},
        )
    ...
    async def _fetch_existing_record(
        ...
        *,
        ...
        if record_level is None:
            return record if int(level) == int(ContextLevel.DETAIL) else None
        return record if int(record_level) == int(level) else None
